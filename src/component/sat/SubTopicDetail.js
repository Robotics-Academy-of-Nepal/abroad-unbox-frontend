    ...
    const fetchContentDetail = async () => {
      try {
        const responseContent = await axios.get(
          `http://192.168.1.29:8000/api/contents/?subtopic_id=${id}`
        );
        ...
        const responseQuizzes = await axios.get(
          `http://192.168.1.29:8000/api/quizzes/?subtopic_id=${id}`
        );
  ...
  return (
    <div className="flex w-full">
      ...
      <div className="flex justify-around">
          ...
          {isAdmin && (<Link to={`/addcontent?subtopic_id=${id}`} className="text-blue-500 underline">Add Content</Link>)}
        </div>
        {/* Content Section */}
        ...
        <div className="my-4">
          ...
          {isAdmin && (
            <Link to={`/addquiz?subtopic_id=${id}`} className="text-blue-500 underline p-6">Add Quiz</Link>
          )}
          {isQuizDropdownOpen && (
